...
async def give_gift(msg):
    ...
    try:
        ...
        await asyncio.sleep(1) # Небольшая пауза для комфорта
...
async def run_test(call: types.CallbackQuery, state: FSMContext):
    ...
    if step < len(QUESTIONS):
        ...
        if score <= 6:
            res_status = "Автор"
            desc = "У вас отличный уровень владения своей жизнью! Чтобы выйти на новый масштаб и калибровать свои цели, предлагаю профессиональный разбор вашего актуального вызова."
        elif score <= 12:
            res_status = "Начинающий Автор"
            desc = "Вы уже на пути к управлению, но иногда внешние сценарии всё еще берут верх. Давайте найдем те точки, где вы отдаете свою силу."
        else:
            res_status = "Заложник обстоятельств"
            desc = "Сейчас кажется, что всё зависит не от вас. Это энергозатратно, но мы можем найти путь возврата к вашей личной силе."
        
        result_text = (
            f"Ваш результат: **{res_status}**\n\n{desc}\n\n"
            "Сформулируйте ваш запрос на бесплатную вводную консультацию (30 мин), и мы проложим ваш авторский маршрут вместе."
        )
...
async def goal(m: types.Message, state: FSMContext):
    ...
    if "не " in m.text.lower():
        return await m.answer("В МПТ мы идем к цели, а не убегаем от проблемы. Попробуйте сформулировать результат без «НЕ». К чему вы хотите прийти?")
...
async def ctrl(m: types.Message, state: FSMContext):
    try:
        ...
        if val < 70:
            return await m.answer(f"Когда ответственность составляет {val}%, управление находится в чужих руках. Попробуйте найти ту грань запроса, где вы будете отвечать за результат на 100%.")
...
async def real(m: types.Message, state: FSMContext):
    if not is_meaningful(m.text): return await m.answer("Опишите ваши будущие действия подробнее.")
...
async def final(m: types.Message, state: FSMContext):
    if not is_meaningful(m.text): return await m.answer("Поделитесь вашим смыслом — почему это действительно важно?")
