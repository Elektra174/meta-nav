...
async def give_gift(chat_id):
    welcome_back = (
        ...
        "🎁 Как и обещал, отправляю вам **полный тест «Свобода быть собой»** в формате PDF (ниже).\n\n"
        "Также предлагаю пройти короткий интерактивный квиз, чтобы увидеть, насколько вы сейчас проявляетесь как Автор своей жизни."
    )
    ...
    try:
        ...
        await bot.send_document(chat_id, document=PDF_GUIDE_URL, caption="Ваш подарок: полный тест 🎁")
...
async def start(msg: types.Message, state: FSMContext):
    ...
    if await check_sub(msg.from_user.id):
        ...
        text = (
            ...
            "Здесь мы исследуем, как перестать бороться с обстоятельствами и обнаружить своё истинное право быть Автором своей жизни.\n\n"
            "🎁 Чтобы забрать подарок (полный тест «Свобода быть собой») и запустить Навигатор, подпишитесь на мой канал."
        )
...
async def run_test(call: types.CallbackQuery, state: FSMContext):
    ...
    if step < len(q):
        ...
        await call.message.answer(f"Вопрос {step+1} из {len(q)}:\n\n{q[step]}", reply_markup=kb)
    else:
        # Оригинальные категории из PDF
        if score <= 6: res = "Автор"
        elif score <= 12: res = "Начинающий Автор"
        elif score <= 18: res = "Заложник"
        else: res = "Жертва"
        
        txt = (
            f"Ваш текущий результат: **{res}**.\n\n"
            "Это отражение того, как вы проявляетесь на данный момент. Хотите исследовать, как это влияет на конкретную сферу вашей жизни?"
        )
